/** Offline copy of GET /api/v1/value-proposition — keeps «القيمة للشركات» full without API. */
...
export type ValuePropositionPayload = {
  tagline_ar: string;
  pillars: ValuePillar[];
  sectors_sample: string[];
  roi_framework_ar: string;
};

export const VALUE_PROPOSITION_FALLBACK: ValuePropositionPayload = {
  ...
  sectors_sample: ["العقارات", "الصحة", "التجزئة", "التعليم", "B2B خدمات"],
  roi_framework_ar:
    "يقيس النظام أثراً مالياً عبر ارتفاع الإيراد، تحسين معدل الفوز، وتسريع الأنبوب مع تقليل العمل اليدوي — جاهز للعرض على الإدارة العليا.",
};
